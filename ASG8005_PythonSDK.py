...
class ASG8005():
    ...
    def __init__(self):
        ...
        self.__dll.isCountContinue.restype = c_int
        self.__dll.isCountContinue.argtypes = [c_int]
    ...
    def checkdata(self, asg_data, length):
        ...
        for i in range(len(asg_data)):
            ...
            for j in range(0,len(asg_data[i])-1,2):
                ...
                if j == 0:
                    if ((asg_data[i][0] < 7.5) and (asg_data[i][0] != 0)) or (asg_data[i][0] > 2600000000.0) or (
                            asg_data[i][1] < 10) or (asg_data[i][1] > 2600000000.0):
                        return bool(False)
                else:
                    if (asg_data[i][j] < 7.5) or (asg_data[i][j] > 2600000000.0) or (asg_data[i][j + 1] < 10) or (
                            asg_data[i][j + 1] > 2600000000.0):
                        return bool(False)
    ...
    def download_ASG_pulse_data(self, asg_data, length):
        ...
        c_asg_data = (c_double  * length[0] * 8)(*(tuple(i) for i in asg_data))
    ...
    def ASG_set_counter_repeat(self,repeat):
        if repeat < 0 :
            exit(" count repeat  error !");
        repeat=repeat*2
        return self.__dll.set_counter_repeat(c_int(repeat))

    def ASG_isCountContinue(self,isContinue):
        if isContinue !=0 and isContinue != 1:
            exit(" count isContinu  error !");
        return self.__dll.isCountContinue(c_int(isContinue))

    def ASG_countTimeStep(self,timeStep):
        if timeStep < 0 :
            exit(" count timeStep  error !");
        return self.__dll.countTimeStep(c_int(timeStep))

    def ASG_countConfig(self,isCountEnable,asgConfig = 0xff):
        if (isCountEnable !=0 and isCountEnable != 1) or (asgConfig<0 or asgConfig>0xff):
            exit(" count Continu  error !");
